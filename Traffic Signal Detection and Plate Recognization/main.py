    ...
    try:
        ...
        if connection.is_connected():
            ...
            if result:
                ...
                cursor.execute(
                    f"UPDATE license_plates SET violation_count=violation_count+1 WHERE plate_number='{plate_number}'")
            else:
...
def main():
    ...
    # Open the video file
    vid = cv2.VideoCapture("F:/Traffic-Violation-Detection/traffic_video.mp4")
...
license_plate_cascade = cv2.CascadeClassifier("F:/Traffic-Violation-Detection/haarcascade_russian_plate_number.xml")
